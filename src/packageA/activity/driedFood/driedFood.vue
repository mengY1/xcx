  ...
  <div>
    ...
    <div :style="subAction?'margin-top:208rpx':''">
      ...
      <main class="mainContent">
        ...
        <div class="copyUrl">
          <p class="queryTime">百度云下载链接：{{enrollUrl}}<span @click="copyUrl">复制</span></p>
          <p class="queryUrl">百度云提取密码：{{password}}<span @click="copyPassword">复制</span></p>
        </div>
        <div class="txt_wrap">
          <p class="content">
            <!--  :class="{'isflod': isFold==true}" -->
            <wxParse :content="detailInfoObj.universities_content" />
          </p>
        </div>
...
export default {
  ...
  onLoad(options) {
    // toShouquan();
    this.scrollTop = 0;
    this.act_id = options.university_id;
  ...
  methods: {
    ...
    collection(e) {
      ...
      actCollection({ type: 2, foreign_id: this.act_id }, true).then(res => {
    ...
    subscriptionAction(e) {
      ...
      if (this.subAction) {
        ...
        subscriptions({ unionid: this.unionid, type: 2 }).then(res => {
          this.guideShow = true;
        });
      }
    },
    ...
    getCommonList() {
      commentsList(
        { pagesize: 1000, page: 1, type: 3, foreign_id: this.act_id },
        "",
        true
      )
        .then(res => {
    ...
    bindKeyInputComment(e) {
      ...
      comments(
        { content: this.content, type: 3, foreign_id: this.act_id },
        this.token,
        false
      ).then(res => {
          wx.showToast({
    ...
    getInfo() {
      this.unionid = wx.getStorageSync("silentlogin").unionid;
      uniDetail(this.act_id, {}, this.token, true)
        .then(res => {
          ...
          this.enrollUrl = `${res.url.substring(0, 17)}...`;
          this.enrollUrlCopy = res.url;
          this.password = res.url_code;
  ...
  onShareAppMessage: function(res) {
    ...
    return {
      ...
      path: this.share_id
        ? "/pages/index/index?share_id= " + this.share_id + "&type=2"
        : "/pages/index/index?university_id=" + this.act_id + "&type=2",
