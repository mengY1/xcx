<template>
  <div>
    ...
    <!-- 订阅按钮 -->
    <div class="subscription_wrap" v-show="subAction">
    ...
    <div :style="subAction?'margin-top:208rpx':''">
      ...
      <main class="mainContent">
        ...
        <div class="copyUrl">
          <p class="queryTime">活动时间：{{detailInfoObj.activity_start_at}}<span :class="{checked : signUpMes == '活动已结束'}" @click="toSignUp">{{signUpMes}}</span></p>
          <p class="queryUrl">活动地点：{{detailInfoObj.address}}</p>
        </div>
        <div class="txt_wrap">
          <p class="content">
    ...
    <footer>
      ...
      <div class="buttonBox" v-if="showPICK">
        ...
        <form @submit="send" report-submit="true" style="margin-right:-24px;">
          <button open-type="share" form-type="submit" @click="shareIdGet"><i class="iconfont icon-share-big"></i></button>
        </form>
        ...
        <form @submit="collection" report-submit="true" v-if="isCollection" style="margin-right:20px;margin-left:15px">
          <button form-type="submit">
            <i class="iconfont icon-Collection"></i>
          </button>
        </form>
        <form @submit="cancelCollection" report-submit="true" v-else style="margin-right:20px;margin-left:15px">
          <button form-type="submit">
...
export default {
  ...
  methods: {
    ...
    subscriptionAction(e) {
      ...
      if (this.subAction) {
        ...
        subscriptions({ unionid: this.unionid, type: 4 }).then(res => {
          this.guideShow = true;
        });
      }
    },
  ...
  onShareAppMessage: function(res) {
    ...
    return {
      ...
      path: this.share_id
        ? "/pages/index/index?share_id=" + this.share_id + "&type=4"
        : "/pages/index/index?act_id=" + this.act_id + "&type=4",
