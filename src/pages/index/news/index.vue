    ...
    <div :style="subAction?'margin-top:208rpx':''">
      ...
      <main class="mainContent">
        ...
        <div class="ql-container ql-snow">
          <div class="ql-editor">
            <div ref="refContent">
              <wxParse :content="detailInfoObj.content" />
            </div>
          </div>
        </div>
      </main>
...
export default {
  ...
  methods: {
    ...
    collection(e) {
      ...
      actCollection({ type: 1, foreign_id: this.act_id }, true).then(res => {
    ...
    subscriptionAction(e) {
      ...
      if (this.subAction) {
        ...
        subscriptions({ unionid: this.unionid, type: 1 }).then(res => {
          this.guideShow = true;
        });
      }
    },
    ...
    getCommonList() {
      commentsList(
        { pagesize: 100, page: 1, type: 2, foreign_id: this.act_id },
        "",
        true
      )
        .then(res => {
    ...
    bindKeyInputComment(e) {
      ...
      comments(
        { content: this.content, type: 2, foreign_id: this.act_id },
        this.token,
        false
      ).then(res => {
        wx.showToast({
    ...
    getInfo() {
      this.unionid = wx.getStorageSync("silentlogin").unionid;
      newsDetail(this.act_id, {}, this.token, true)
        .then(res => {
  ...
  onShareAppMessage: function(res) {
    ...
    return {
      ...
      path: this.share_id
        ? "/pages/index/index?share_id= " + this.share_id + "&type=1"
        : "/pages/index/index?act_id=" + this.act_id + "&type=1"
